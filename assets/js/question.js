...
axios
  ...
  .then((response) => {
    if (
      ...
      document.getElementById(
        "question-container"
      ).innerHTML = `<div class="error-message">Savollarni olishda xato yuz berdi. Iltimos, qayta urinib ko'ring.</div>`;
      return;
    }
  ...
  .catch((error) => {
    document.getElementById(
      "question-container"
    ).innerHTML = `<div class="error-message">Savollarni olishda xato yuz berdi: ${error.message}</div>`;
  });
...
function handleAnswer(answerOption, questionData) {
  document.querySelectorAll(".answer-option").forEach((option) => {
    option.classList.add(
      option.dataset.correct === "true" ? "correct" : "wrong"
    );
...
function showFinalResult() {
  ...
  updateBestScore(correctAnswersCount);
  updateGameCount() 
}
...
function updateScore(count) {
  axios
    ...
    .then((res) => {
      const oldScore = res.data.totalOfFindAnswerAll;
      const totalCount = oldScore + count;
      return axios.put(
        `https://678b9aa91a6b89b27a2ae07d.mockapi.io/quiz/${userId}`,
        {
          totalOfFindAnswerAll: totalCount,
        }
      );
    });
}
function updateGameCount() {
  axios
    .get(`https://678b9aa91a6b89b27a2ae07d.mockapi.io/quiz/${userId}`)
    .then((res) => {
      const olGameC = res.data.totalOfGame;
      const totalGameCount = olGameC + 1;
      return axios.put(
        `https://678b9aa91a6b89b27a2ae07d.mockapi.io/quiz/${userId}`,
        {
            totalOfGame: totalGameCount,
        }
      );
    });
}

function updateBestScore(count) {
  axios
    .get(`https://678b9aa91a6b89b27a2ae07d.mockapi.io/quiz/${userId}`)
    .then((res) => {
      const oldBestScore = res.data.bestScore;
      if (count > oldBestScore) {
        return axios.put(
          `https://678b9aa91a6b89b27a2ae07d.mockapi.io/quiz/${userId}`,
          {
            bestScore:count,
